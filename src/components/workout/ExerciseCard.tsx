...
type Props = { exercise: Exercise; onChange: (exercise: Exercise) => void; onDelete: () => void; collapsed?: boolean; onToggle?: () => void; onAddSet?: () => void };

export default function ExerciseCard({ exercise, onChange, onDelete, collapsed = false, onToggle, onAddSet }: Props) {
  const previous = getPreviousPerformance(exercise.name);
  const { start } = useRestTimerStore();
  const { settings } = useSettingsStore();
  const completedSets = exercise.sets.filter(set => set.completed).length;
  const exerciseCompleted = exercise.sets.length > 0 && completedSets === exercise.sets.length;

  function updateSet(index: number, values: Partial<WorkoutSet>) {
  ...
  function completeSet(index: number) {
    ...
    if (completed && settings.autoStartRestTimer) start(settings.defaultRestTimer);
  }
  function addSet() {
    const last = exercise.sets[exercise.sets.length - 1];
    onChange({ ...exercise, sets: [...exercise.sets, { id: crypto.randomUUID(), weight: last?.weight ?? 0, reps: last?.reps ?? 0, completed: false }] });
    onAddSet?.();
  }
  ...
  return (
    <Card className={["p-3.5", exerciseCompleted ? "border border-green-500/70" : ""].join(" ")}>
      <div className="flex items-start justify-between gap-3">
        <button type="button" onClick={onToggle} className="min-w-0 flex-1 text-left">
          <div className="flex items-center gap-2">{exerciseCompleted && <span className="text-green-400">✓</span>}<h2 className="truncate text-[17px] font-semibold">{exercise.name}</h2>{collapsed ? <ChevronDown size={18} className="shrink-0 text-zinc-500" /> : <ChevronUp size={18} className="shrink-0 text-zinc-500" />}</div>
          <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-zinc-500"><span>{exercise.muscleGroup}</span><span>•</span><span>{completedSets}/{exercise.sets.length} sets</span>{previous && previous.sets.length > 0 && <><span>•</span><span>Last • {previous.sets[0].weight} kg × {previous.sets[0].reps}</span></>}</div>
        </button>
