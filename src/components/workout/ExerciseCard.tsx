      ...
      <div className="flex items-start justify-between gap-3">
        <button type="button" onClick={onToggle} className="min-w-0 flex-1 text-left">
          <div className="flex items-center gap-2">
            {exerciseCompleted && <span className="text-green-400">✓</span>}
            <h2 className="truncate text-[17px] font-semibold">{exercise.name}</h2>
            {collapsed ? <ChevronDown size={18} className="shrink-0 text-zinc-500" /> : <ChevronUp size={18} className="shrink-0 text-zinc-500" />}
          </div>
          <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-zinc-500">
            <span>{exercise.muscleGroup}</span><span>•</span><span>{completedSets}/{exercise.sets.length} sets</span>
